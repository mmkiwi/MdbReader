...
using static MMKiwi.MdbTools.Mutable.MdbBuilder;
...
public partial class Jet3Reader
{
    internal class LvalStream
    {
        internal LvalStream(Jet3Reader reader, MdbColumn column, ImmutableArray<byte> firstDataPointer)
        {
            ...
            Length = unchecked((int)
                  (BinaryPrimitives.ReadUInt32LittleEndian(binRowRegion)
            ...
            {
                LvalPointer = BinaryPrimitives.ReadInt32LittleEndian(FirstDataPointer.AsSpan().Slice(4, 4));
            }
        }
